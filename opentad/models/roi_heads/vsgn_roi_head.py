...
class VSGNRoIHead(nn.Module):
    ...
    def forward_train(self, feats, roi_bounds, gt_segments=None, gt_labels=None, return_loss=True):
        ...
        if return_loss:
            ...
            gts = [torch.cat((gt_b, gt_l[:, None] ), dim=-1) for gt_b, gt_l in zip(gt_segments, gt_labels)]  # gts = [torch.cat((gt_b, gt_l[:, None] + 1), dim=-1) for gt_b, gt_l in zip(gt_segments, gt_labels)]
