...
#         create_folder(cfg.inference["folder"])
#
#     # --- 修改 1: 修复 external_cls 未定义报错 ---
#     external_cls = None  # 先给一个默认值，防止后面报错
#
#     if "external_cls" in cfg.post_processing:
#         if cfg.post_processing.external_cls != None:
#             external_cls = build_classifier(cfg.post_processing.external_cls)
#     else:
#         # 如果配置里没写 external_cls，才使用 dataset 的 map (虽然这对 BMN 可能没用)
#         if hasattr(test_loader.dataset, 'class_map'):
#             external_cls = test_loader.dataset.class_map
#     # ----------------------------------------
#
#     # whether the testing dataset is sliding window
...
#
#         # --- 修改 2: 强力诊断 mAP=0 问题 ---
#         # 我们只打印第一个 batch 的第一个结果，避免刷屏
#         if rank == 0 and not debug_printed and len(results) > 0:
#             debug_printed = True
#             print("\n\n" + "=" * 50)
#             print(">>> DEBUG START: 检查预测输出 <<<")
#
#             # 获取第一个视频的结果
#             first_vid_name = list(results.keys())[0]
#             preds = results[first_vid_name]
#
#             print(f"1. 视频名称: {first_vid_name}")
#             print(f"2. 预测框数量: {len(preds)}")
#
#             if len(preds) > 0:
#                 # 打印前 3 个预测框详情
#                 print("3. 前3个预测框详情 (检查 segment 单位和 score):")
#                 for i, p in enumerate(preds[:3]):
#                     print(f"   Pred {i}: {p}")
#
#                 # 智能分析单位
#                 seg_sample = preds[0]['segment']
#                 if seg_sample[1] > 1000:
#                     print(">>> 分析: 数值很大 (>1000)，可能是【帧索引 (Frames)】")
#                     print(">>> 警告: 如果 GT 是秒，这里会导致 IoU=0")
#                 elif seg_sample[1] < 1.0:
#                     print(">>> 分析: 数值很小 (<1.0)，可能是【归一化坐标】")
#                 else:
#                     print(">>> 分析: 数值正常，看起来像是【秒 (Seconds)】")
#             else:
#                 print(">>> 警告: 模型没有输出任何预测框！可能是阈值太高或分类分数为0")
#
#             print("=" * 50 + "\n\n")
#         # ----------------------------------------
#
