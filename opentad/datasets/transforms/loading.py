...
class LoadFeats:
    # 1. 为了训练dyfadet 修改：在参数里添加 normalize=False，作为默认关闭的开关
    def __init__(self, feat_format, prefix="", suffix="", normalize=False):
        self.feat_format = feat_format
        self.normalize = normalize  # 这里接收参数并保存
        self.prefix = prefix
        self.suffix = suffix
    ...
    def __call__(self, results):
        ...
        # ==========================================
        # 2. 新增逻辑：PP-TSM 特征 L2 归一化核心代码
        # ==========================================
        if self.normalize:
            # 必须先转成 tensor 才能用 F.normalize
            feats_tensor = torch.from_numpy(feats)
            # p=2 表示 L2 范数，dim=1 表示对通道维度 (Channel) 进行归一化
            feats_tensor = F.normalize(feats_tensor, p=2, dim=1)
    ...
    def __repr__(self):
        repr_str = f"{self.__class__.__name__}(" f"feat_format={self.feat_format}, normalize={self.normalize})"
        return repr_str
